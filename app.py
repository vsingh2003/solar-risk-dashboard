"""
Solar Risk Monitor — Dashboard v2
...
<style>
    ...
    /* ── Plotly chart backgrounds already handled via template ── */
...
try:
    ...
    st.error("⚠️ **final_anomaly_results.csv not found.**  "
...
if len(date_range) == 2:
    fdf = fdf[
        (fdf['date'] >= pd.Timestamp(date_range[0])) &
...
# ══════════════════════════════════════════════════════════════
#  SECTION 1 — KPI METRICS ROW (4 columns)
# ══════════════════════════════════════════════════════════════
...
anom_rate        = (total_anomalies / len(fdf) * 100) if len(fdf) else 0

# Highest risk unit across the FULL dataset (not filtered)
...
# ══════════════════════════════════════════════════════════════
#  SECTION 2 — DAILY YIELD TREND (Plotly, dark)
# ══════════════════════════════════════════════════════════════
...
if len(fdf) == 0:
    ...
    if selected_unit == 'All Units':
        # Show one line per unit
        trend_df = (fdf.groupby(['date','unit_id'], as_index=False)
                       .agg(daily_yield_kwh=('daily_yield_kwh','mean')))
        fig_trend = px.line(
            trend_df, x='date', y='daily_yield_kwh', color='unit_id',
            template=PLOTLY_DARK,
            labels={'daily_yield_kwh':'Daily Yield (kWh)',
                    'date':'Date', 'unit_id':'Unit'},
            title="Daily Energy Yield — All Units"
        )
    else:
        ...
        fig_trend.update_layout(
            template=PLOTLY_DARK,
            title=f"Daily Yield — {selected_unit}",
            xaxis_title='Date', yaxis_title='Daily Yield (kWh)',
...
# ══════════════════════════════════════════════════════════════
#  SECTION 3 — UNIT COMPARISON CHARTS (2 columns)
# ══════════════════════════════════════════════════════════════
...
with col_left:
    ...
    unit_avg = (df.groupby('unit_id', as_index=False)
...
with col_right:
    ...
    unit_anom = (df.groupby('unit_id', as_index=False)
    ...
    fig_anom_bar = px.bar(
        ...
        title="Anomalies Detected per Unit"
    )
...
# ══════════════════════════════════════════════════════════════
#  SECTION 4 — CORRELATION SCATTER PLOT (new)
# ══════════════════════════════════════════════════════════════
st.markdown("## 🔬 Correlation Analysis — Key Metrics")
st.caption("Each dot is one unit-day. Red dots = flagged anomalies. "
...
with col_s1:
    x_axis = st.selectbox(
        "X-axis metric",
        ['rolling_mean_7', 'z_score', 'pct_dev_from_fleet',
         'day_change_pct', 'performance_ratio', 'ratio_to_rollmean'],
        index=0
    )
with col_s2:
    y_axis = st.selectbox(
        "Y-axis metric",
        ['daily_yield_kwh', 'z_score', 'pct_dev_from_fleet',
         'day_change_pct', 'performance_ratio', 'rolling_std_7'],
        index=0
    )
...
if len(scatter_df) > 1:
    fig_scatter = px.scatter(
        ...
        symbol='unit_id',
        template=PLOTLY_DARK,
        title=f"{y_axis}  vs  {x_axis}  — coloured by anomaly status",
...
| **performance_ratio** | Low | Poor yield relative to installed capacity |
| **ML flag** | — | Isolation Forest detected unusual pattern |

**🟢 Normal** · **🟡 Mild concern** · **🔴 Flagged anomaly**
    """)

st.markdown("---")

# ══════════════════════════════════════════════════════════════
#  SECTION 6 — FLAGGED RECORDS TABLE
# ══════════════════════════════════════════════════════════════
...
if anomaly_view.empty:
    ...
    summary = (anomaly_view.groupby('unit_id', as_index=False)
               .agg(Anomalies=('anomaly_flag','sum'),
                    Avg_Yield=('daily_yield_kwh','mean'),
                    Avg_Fleet_Dev=('pct_dev_from_fleet','mean'))
               .rename(columns={'unit_id':'Unit',
                                'Avg_Yield':'Avg Yield (kWh)',
                                'Avg_Fleet_Dev':'Avg Fleet Dev (%)'}))
    summary['Avg Yield (kWh)']   = summary['Avg Yield (kWh)'].round(2)
    summary['Avg Fleet Dev (%)'] = summary['Avg Fleet Dev (%)'].round(1)
    st.dataframe(summary, use_container_width=True, hide_index=True)

    st.markdown("##### All Flagged Records")
    display_df = anomaly_view[[
        'date','unit_id','source','daily_yield_kwh',
        'pct_dev_from_fleet','z_score','day_change_pct',
        'rule_anomaly','ml_anomaly','explanation'
    ]].copy()
    display_df.columns = [
        'Date','Unit','Source','Yield (kWh)',
        'Fleet Dev (%)','Z-Score','Day Chg (%)','Rule','ML','Explanation'
    ]
...
st.caption("Try: 'most anomalies' · 'best unit' · 'average yield' · "
           "'total anomalies' · 'how many units' · 'date range' · 'worst unit'")

query = st.text_input(
    "Type your question:",
    placeholder="e.g.  which unit has the most anomalies?"
)
...
if query:
    ...
    elif any(w in q for w in ['visser','bissell','source']):
        src_counts = df.groupby('source')['anomaly_flag'].sum()
        msg = "  \n".join([f"**{s}**: {int(c)} anomalies" for s, c in src_counts.items()])
        st.info(f"📊 Anomalies by data source:\n{msg}")

    else:
        st.warning(
            "I can answer: **most anomalies · worst/best unit · average yield · "
            "total anomalies · how many units · date range · fleet average · "
            "visser/bissell breakdown**"
        )
...
st.markdown(
    ...
    "⚡ Solar Risk Monitor &nbsp;·&nbsp; Anomaly Detection &amp; Risk Monitoring "
