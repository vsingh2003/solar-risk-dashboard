"""
Solar Risk Monitor — Dashboard v3 (Tabbed Layout for Deliverables)
...
# TABS SETUP
# ══════════════════════════════════════════════════════════════
tab1, tab2, tab3 = st.tabs(["📈 Live Dashboard", "📊 Exploratory Data Analysis (EDA)", "🧠 Modeling & XAI"])
...
with tab1:
    ...
    if len(fdf) == 0:
        ...
        if selected_unit == 'All Units':
            trend_df = (fdf.groupby(['date','unit_id'], as_index=False).agg(daily_yield_kwh=('daily_yield_kwh','mean')))
            fig_trend = px.line(trend_df, x='date', y='daily_yield_kwh', color='unit_id', template=PLOTLY_DARK, labels={'daily_yield_kwh':'Daily Yield (kWh)', 'date':'Date', 'unit_id':'Unit'})
        else:
            normal_df = fdf[fdf['anomaly_flag'] == 0]
            anom_df   = fdf[fdf['anomaly_flag'] == 1]
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Scatter(x=fdf['date'], y=fdf['daily_yield_kwh'], mode='lines', name='Daily Yield', line=dict(color='#58a6ff', width=2)))
            fig_trend.add_trace(go.Scatter(x=fdf['date'], y=fdf['rolling_mean_7'], mode='lines', name='7-Day Rolling Mean', line=dict(color='#d29922', width=1.5, dash='dash')))
            fig_trend.add_trace(go.Scatter(x=anom_df['date'], y=anom_df['daily_yield_kwh'], mode='markers', name='⚠ Anomaly', marker=dict(color='#f85149', size=10, symbol='x', line=dict(width=2, color='#f85149'))))
            
        fig_trend.update_layout(paper_bgcolor=PLOTLY_PAPER, plot_bgcolor=PLOTLY_BG, height=400, margin=dict(l=20, r=20, t=20, b=20))
    ...
    if anomaly_view.empty:
        ...
        display_df = anomaly_view[['date','unit_id','source','daily_yield_kwh','pct_dev_from_fleet','z_score','day_change_pct','explanation']].copy()
        display_df.columns = ['Date','Unit','Source','Yield (kWh)','Fleet Dev (%)','Z-Score','Day Chg (%)','Explanation']
...
with tab2:
    ...
    with col_left:
        ...
        unit_avg = (df.groupby('unit_id', as_index=False).agg(avg_yield=('daily_yield_kwh','mean')).sort_values('avg_yield', ascending=True))
        fig_bar = px.bar(unit_avg, x='avg_yield', y='unit_id', orientation='h', color='avg_yield', color_continuous_scale='Blues', template=PLOTLY_DARK, labels={'avg_yield':'Avg Daily Yield (kWh)', 'unit_id':'Unit'})
        fig_bar.update_layout(paper_bgcolor=PLOTLY_PAPER, plot_bgcolor=PLOTLY_BG, height=320, margin=dict(l=10,r=10,t=10,b=10), coloraxis_showscale=False)
        st.plotly_chart(fig_bar, use_container_width=True)

    with col_right:
        st.markdown("### Anomaly Count by Unit")
        unit_anom = (df.groupby('unit_id', as_index=False).agg(anomalies=('anomaly_flag','sum')).sort_values('anomalies', ascending=True))
        fig_anom_bar = px.bar(unit_anom, x='anomalies', y='unit_id', orientation='h', color='anomalies', color_continuous_scale='Reds', template=PLOTLY_DARK, labels={'anomalies':'Anomaly Count', 'unit_id':'Unit'})
        fig_anom_bar.update_layout(paper_bgcolor=PLOTLY_PAPER, plot_bgcolor=PLOTLY_BG, height=320, margin=dict(l=10,r=10,t=10,b=10), coloraxis_showscale=False)
    ...
    with col_s1:
        x_axis = st.selectbox("X-axis metric", ['rolling_mean_7', 'z_score', 'pct_dev_from_fleet', 'day_change_pct', 'performance_ratio', 'ratio_to_rollmean'], index=0)
    with col_s2:
        y_axis = st.selectbox("Y-axis metric", ['daily_yield_kwh', 'z_score', 'pct_dev_from_fleet', 'day_change_pct', 'performance_ratio', 'rolling_std_7'], index=0)

    if len(scatter_df) > 1:
        fig_scatter = px.scatter(scatter_df, x=x_axis, y=y_axis, color='Status', color_discrete_map={'Normal':'#3fb950', 'Anomaly':'#f85149'}, hover_data=['date','unit_id','daily_yield_kwh','explanation'], symbol='unit_id', template=PLOTLY_DARK, labels={'Status':'Classification'})
        fig_scatter.update_traces(marker=dict(size=7, opacity=0.8))
        fig_scatter.update_layout(paper_bgcolor=PLOTLY_PAPER, plot_bgcolor=PLOTLY_BG, height=430, margin=dict(l=20,r=20,t=20,b=20))
        st.plotly_chart(fig_scatter, use_container_width=True)
    else:
        st.info("Not enough data points in current filter to render scatter plot.")

# ==============================================================
# TAB 3: MODELING & XAI
# ==============================================================
with tab3:
    st.markdown("## 🧠 Modeling Approach & Explainable AI")
    st.info("**Model Overview:** This pipeline uses a hybrid anomaly detection system combining **Rule-Based Heuristics** (to catch severe, obvious drops) and an **Isolation Forest** (an unsupervised Machine Learning algorithm to detect complex, multi-dimensional anomalies in the sensor data).")
    
    col_fi, col_guide = st.columns([1, 1])

    with col_fi:
        if feat_imp_df is not None:
            fig_fi = px.bar(feat_imp_df.sort_values('importance', ascending=True), x='importance', y='feature', orientation='h', color='importance', color_continuous_scale='Purples', template=PLOTLY_DARK, title="Which signals matter most?", labels={'importance':'Importance Score','feature':'Feature'})
            fig_fi.update_layout(paper_bgcolor=PLOTLY_PAPER, plot_bgcolor=PLOTLY_BG, height=340, margin=dict(l=10,r=10,t=50,b=10), coloraxis_showscale=False)
            st.plotly_chart(fig_fi, use_container_width=True)
        else:
            st.info("Run pipeline.py to generate feature_importance.csv")
...
st.markdown("---")
st.markdown("<div style='text-align:center; color:#30363d; font-size:0.78rem; padding:8px'>⚡ Solar Risk Monitor &nbsp;·&nbsp; Built with Python, Streamlit &amp; Plotly</div>", unsafe_allow_html=True)